...
volatile uint32_t pulseDuration = 0; // Measured duration in microseconds
volatile uint8_t edgeDetected = 0;   // 0: Waiting for rising edge, 1: Waiting for falling edge
volatile uint32_t lastEdgeTime = 0;  // Timestamp of the last edge detection
const uint32_t timeout = 1000;       // Timeout in microseconds (adjust as needed)
/* USER CODE END PD */
